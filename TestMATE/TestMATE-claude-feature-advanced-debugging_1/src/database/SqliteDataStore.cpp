...
CResult CSqliteDataStore::GetTestData(TUInt64 in_id, STestDataRecord& out_record) {
    ...
    const char* sql = "SELECT id, sequence_name, lot_id, device_id, operator_name, verdict, duration_ms FROM test_data WHERE id = ?";
    sqlite3* db = static_cast<sqlite3*>(m_pDatabase);
    sqlite3_stmt* stmt = nullptr;
...
SQueryResult CSqliteDataStore::QueryParameterized(const TString& in_strQuery,
    ...
    int rc = sqlite3_prepare_v2(static_cast<sqlite3*>(m_pDatabase), in_strQuery.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        result.errorMessage = "Failed to prepare query: " + TString(sqlite3_errmsg(static_cast<sqlite3*>(m_pDatabase)));
        return result;
    }

    // Bind parameters (uses 1-based indexing in SQLite)
