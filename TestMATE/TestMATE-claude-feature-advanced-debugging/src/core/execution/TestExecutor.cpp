...
CResult CTestExecutor::ExecuteStep(ITestStep* io_pStep, TUInt32 in_uiIndex) {
    ...
    // Debug: Notify step entry
    if (m_debugSession) {
        m_debugSession->OnStepEnter(io_pStep->GetId(), io_pStep->GetName());

        // Check if debugger wants to break here
        if (m_debugSession->ShouldBreak()) {
            auto action = m_debugSession->WaitForDebugAction();

            // Handle debug action
            if (action == EDebugAction::kAbort) {
                m_bAbortRequested = true;
                return TESTMATE_FAILURE(EErrorCode::kStepAborted, "Debug abort requested");
            }
        }
    }
    ...
    // Debug: Notify step exit
    if (m_debugSession) {
        m_debugSession->OnStepExit(io_pStep->GetId());
    }
...
void CTestExecutor::SetDebugSession(std::shared_ptr<CDebugSession> in_debugSession) {
    m_debugSession = std::move(in_debugSession);
}
