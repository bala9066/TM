...
void CPerformanceProfiler::StopProfiling(const TString& in_strName) {
    ...
    if (it == m_activeProfiles.end()) {
        LOG_WARNING("No active profiling found for: " + in_strName);
        return;
    }
    ...
    LOG_DEBUG("Stopped profiling: " + in_strName +
             " (" + std::to_string(measurement.durationNs / 1000.0) + " μs)");
}
...
void CPerformanceProfiler::Checkpoint(const TString& in_strName) {
    ...
    LOG_INFO("Performance checkpoint: " + in_strName);
    PrintSummary();
}
...
CResult CPerformanceProfiler::GenerateHtmlReport(const TString& in_strFilePath,
    ...
    try {
        ...
        if (!file.is_open()) {
            return TESTMATE_ERROR(EErrorCode::kFileWriteFailed,
        ...
        LOG_INFO("Performance report generated: " + in_strFilePath);
...
CResult CPerformanceProfiler::GenerateJsonReport(const TString& in_strFilePath,
    ...
    try {
        ...
        LOG_INFO("Performance report (JSON) generated: " + in_strFilePath);
        return TESTMATE_SUCCESS();
    }
    catch (const std::exception& e) {
        return TESTMATE_ERROR(EErrorCode::kFileWriteFailed, e.what());
...
CResult CPerformanceProfiler::GenerateCsvReport(const TString& in_strFilePath) {
    try {
        ...
        file.close();
        LOG_INFO("Performance report (CSV) generated: " + in_strFilePath);
        return TESTMATE_SUCCESS();
    }
    catch (const std::exception& e) {
        return TESTMATE_ERROR(EErrorCode::kFileWriteFailed, e.what());
...
void CPerformanceProfiler::Clear() {
    ...
    LOG_INFO("Performance profiler cleared");
}
...
void CPerformanceProfiler::Reset() {
    ...
    std::lock_guard<std::mutex> lock(m_mutex);
    m_baselines.clear();
    LOG_INFO("Performance profiler reset");
}
...
void CPerformanceProfiler::SetBaseline(const TString& in_strName, TUInt64 in_baselineNs) {
    ...
    LOG_INFO("Baseline set for " + in_strName + ": " +
            std::to_string(in_baselineNs / 1000.0) + " μs");
}
